...
import type { CategoryFilterInput, CategoryFilterOptions, ProductDetails } from '../../model/products';

const productService = {
  // Obtém todos os produtos do catálogo.
  async getAll(limit = 20, offset = 0) {
    const pagination = productService.parsePagination(limit, offset);
  ...
  async getByCategory(categoryId: number, limit = 20, offset = 0, filterOptions: CategoryFilterInput = {}) {
    const pagination = productService.parsePagination(limit, offset);
    const filters = productService.buildCategoryFilters(categoryId, filterOptions);
    const result = await pool.query(
      `SELECT *, COUNT(*) OVER()::int AS total_count
      FROM product
      WHERE ${filters.where}
      ${productService.buildOrderBy(filters.sort)}
  ...
  async getCategoryFilterOptions(categoryId: number): Promise<CategoryFilterOptions> {
    const result = await pool.query(
  ...
  async getDetails(id: number): Promise<ProductDetails | null> {
  ...
  async search(searchQuery, limit = 20, offset = 0) {
    const pagination = productService.parsePagination(limit, offset);
    const result = await pool.query(
  ...
  buildCategoryFilters(categoryId: number, filterOptions: CategoryFilterInput = {}) {
