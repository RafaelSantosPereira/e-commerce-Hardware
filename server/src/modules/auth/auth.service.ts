...
import type { AuthUser, LoginResult, RegisterData } from '../../model/auth';
...
const authService = {
  ...
  async register({ name, email, password, role }: RegisterData): Promise<void> {
  ...
  async resendVerification(email) {
    ...
    await authService.sendVerificationEmail(email, user.name, token);
  },
  ...
  async getSession(userId: number): Promise<Pick<AuthUser, 'name' | 'role'> | null> {
    const result = await pool.query(
