...
import type { MainLayoutContext, Product, PaginatedProductsResponse } from '@/types';
...
function Home() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${apiUrl}/products/category/id/2?limit=${limit}`).then(res => res.json() as Promise<PaginatedProductsResponse>), // Placas Gráficas
      fetch(`${apiUrl}/products/category/id/1?limit=${limit}`).then(res => res.json() as Promise<PaginatedProductsResponse>), // Processadores
      fetch(`${apiUrl}/products/category/id/4?limit=${limit}`).then(res => res.json() as Promise<PaginatedProductsResponse>)  // Memórias RAM
    ])
